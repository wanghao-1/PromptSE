...
parser.add_argument('--data_path', type=str, default='/data/', help='pa  th to data')
...
train_val_index, test_index = train_test_split(np.arange(len(data_set)), test_size=0.1, random_state=10)
train_index, valid_index = train_test_split(train_val_index, test_size=0.05, random_state=10)
...
edge_index_to_drug_drug = torch.tensor(get_edge_index_and_weights(data_set_to_drug_drug)[0], dtype=torch.int32).to(device)
weight_drug=torch.tensor(get_edge_index_and_weights(data_set_to_drug_drug)[1],dtype=torch.float32).to(device)

data_set_to_se_se=process_matrix(data_set_to_se_se,args.kk)
edge_index_to_se_se = torch.tensor(get_edge_index_and_weights(data_set_to_se_se)[0], dtype=torch.int64).to(device)
weight_se=torch.tensor(get_edge_index_and_weights(data_set_to_se_se)[1],dtype=torch.float32).to(device)
...
model = DSEModel(args, fpt_feature, mpnn_feature, weave_feature, afp_feature, nf_feature, vec_feature, edge_index_to_drug_drug,
